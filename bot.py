...
def next_tonnel_proxies() -> dict | None:
    ...
    каждый вызов tonnel_listings_by_sort/tonnel_auctions/tonnel_sales_history
    берёт следующий адрес из списка.

    Вызывается из потоков пула asyncio.to_thread (внутри синхронных функций
    tonnelmp), а не из основного event loop — поэтому инкремент защищён
    Lock'ом: без него при параллельном сканировании нескольких коллекций
    (см. SCAN_CONCURRENCY) счётчик мог бы гонки терять обновления между
    потоками, и ротация работала бы неровно."""
    if not _PROXY_URLS:
        return None
...
async def tonnel_listings_by_sort(name: str, sort: str) -> tuple[list[Listing], str | None]:
    def _call():
        return tonnelmp.getGifts(gift_name=name, sort=sort, limit=30, asset="TON", proxies=next_tonnel_proxies())
    try:
...
async def tonnel_auctions(name: str) -> tuple[list[AuctionItem], str | None]:
    def _call():
        return tonnelmp.getAuctions(gift_name=name, sort="latest", limit=AUCTIONS_LIMIT, asset="TON", proxies=next_tonnel_proxies())
    try:
        raw = await asyncio.to_thread(_call)
    except Exception as e:
        err = mask_proxy_creds(e)
...
async def tonnel_sales_history(name: str) -> tuple[list[SaleRecord], str | None]:
    def _call():
        return tonnelmp.saleHistory(authData="", gift_name=name, limit=SALES_HISTORY_LIMIT, sort="latest", proxies=next_tonnel_proxies())
    try:
        raw = await asyncio.to_thread(_call)
    except Exception as e:
        err = mask_proxy_creds(e)
        log.warning(f"saleHistory('{name}') не удался: {err}")
        return [], err
...
async def main():
    ...
    if not _PROXY_URLS:
        ...
        log.info(f"Прокси для Tonnel настроен: {len(_PROXY_URLS)} адрес(ов) (round-robin)")
