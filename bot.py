...
async def tonnel_listings_by_sort(name: str, sort: str) -> tuple[list[Listing], str | None]:
    def _call():
        return tonnelmp.getGifts(gift_name=name, sort=sort, limit=30, asset="TON")
    try:
    ...
    listings = []
    for it in raw or []:
        if it.get("status") != "forsale" or not it.get("price") or not it.get("gift_id"):
            continue
...
async def tonnel_auctions(name: str) -> tuple[list[AuctionItem], str | None]:
    def _call():
        return tonnelmp.getAuctions(gift_name=name, sort="latest", limit=AUCTIONS_LIMIT, asset="TON")
    try:
        raw = await asyncio.to_thread(_call)
    except Exception as e:
    ...
    if match and match.price_ton > 0:
        ...
        if discount_vs_last_sale >= threshold_pct:
            ...
            reasons.append(
                ...
                f"({model} / {backdrop} / {symbol}) на {discount_vs_last_sale:.0f}% "
...
async def scan_collection(bot: Bot, tc: TrackedCollection, manual: bool = False) -> dict:
    ...
    if tc.consecutive_errors >= CONSECUTIVE_ERRORS_BEFORE_NOTIFY and not tc.error_notified and not manual:
        ...
        await bot.send_message(
            tc.chat_id,
            f"⚠️ «{tc.name}»: {tc.consecutive_errors} раз(а) подряд не получилось "
            f"получить данные с Tonnel (возможно, 429/403 от их API или сменилось "
            f"название коллекции). Последняя ошибка: {errors[0] if errors else 'неизвестна'}.\n"
...
def confirmation_text(name: str, threshold: float, max_price: float | None) -> str:
    ...
    return (
        f"✅ Слежу за «{name}», алерт от {threshold:.0f}% дешевле рынка, {limit_text}.\n"
...
async def on_threshold(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        f"«{name}», алерт от {threshold:.0f}% дешевле рынка.\n\n"
...
async def on_check_now(callback: CallbackQuery, bot: Bot):
    ...
    if stats["errors"]:
        ...
        lines.extend(f"— {e}" for e in stats["errors"])
